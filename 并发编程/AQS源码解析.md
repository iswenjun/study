...
执行 `acquire()` 方法阻塞，直到有一个许可证可以获得然后拿走一个许可证；每个 `release` 方法增加一个许可证，这可能会释放一个阻塞的 `acquire()` 方法。然而，其实并没有实际的许可证这个对象，`Semaphore` 只是维持了一个可获得许可证的数量。 `Semaphore` 经常用于**限制获取某种资源的线程数量**。
...
| 方法                              | 说明                       |
| :-------------------------------- | :------------------------- |
| boolean tryAcquire(int arg)       | 尝试以独占模式获取         |
...
[AbstractQueuedSynchronizer 原理分析 - 独占/共享模式 - 云+社区 - 腾讯云 (tencent.com)](https://cloud.tencent.com/developer/article/1113761)
